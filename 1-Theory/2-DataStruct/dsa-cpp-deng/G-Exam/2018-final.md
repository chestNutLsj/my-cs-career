...
> ❌ 这里错误原因并非是“不超过 $O(e)$ ”这句话。
> [[81-Graph-Application#优先级搜索]]
> 事实上，如果用邻接表实现，`prioUpdater()` 只需要常数时间，而前一内循环的累积时间取决于所有顶点的出度之和，为 $O(e)$，此时这句话将是正确的。
> 但是若图通过邻接矩阵实现，`prioUpdater()` 却需要 $O(n)$ 时间，此时前一内循环的时间复杂度将是 $O(n^{2})$。
...
> 假设 $\forall k\ge 2,\phi_{k-2}\phi_{k-1}=\phi_{k}^{'}$ 成立，那么
