...
```
这里的 clear ()接口并非简单地破坏原校验环，而是将 T\[ F\[k\] \]取负之后再减一。也就是说，可以在与正常校验环不相冲突的前提下，就地继续保留原校验环的信息。基于这一约定，set (k)接口只需调用 erase (k)，即可简明地判定\[k\]究竟属于哪种类型。若系从未标记过的，则按此前的方法新建一个校验环；否则可以直接恢复原先的校验环。
