...
- virt 表示了一台虚拟的 RISC-V 64 计算机，CPU 的个数是可以通过参数 `-cpu num` 配置的，内存也是可通过参数 `-m numM/G` 来配置。
...
- 我们只需要给 main 函数增加两个参数（即 `a0` 和 `a1` 寄存器中的值 ）即可，这样测试用例就获得了 bootloader 传来的放置 DTB 的物理地址。
