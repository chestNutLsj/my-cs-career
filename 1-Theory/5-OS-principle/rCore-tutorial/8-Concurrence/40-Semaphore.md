...
- 第 22 行我们调用 `semaphore_create` 函数创建一个用于同步的 的信号量，其 ID 应为 `SEM_SYNC` 。
...
`new` 方法比较简单。而 `up` 和 `down` 方法和我们在信号量基本思路中介绍的 [[#^ace2a9|信号量的第二种实现]]) 一致。只需要注意如何使用阻塞 - 唤醒机制的核心接口 `block_current_and_run_next` 和 `wakeup_task` 即可。
