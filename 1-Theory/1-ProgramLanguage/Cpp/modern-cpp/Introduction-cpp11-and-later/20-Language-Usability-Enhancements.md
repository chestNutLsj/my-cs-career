...
为此，我们可以写出下面这类简化的版本 (三元运算符) 来使得函数从 C++11 开始即可用：
...
结构化绑定**提供了类似其他语言中提供的多返回值的功能**。在容器一章中，我们会学到 C++11 新增了 `std::tuple` 容器用于构造一个元组，进而囊括多个返回值。但缺陷是，C++11/14 并没有提供一种简单的方法直接从元组中拿到并定义元组中的元素，尽管我们可以使用 `std::tie` 对元组进行拆包，但我们依然必须非常清楚这个元组包含多少个对象，各个对象是什么类型，非常麻烦。
...
关于 `auto` 类型推导会在 [[20-Language-Usability-Enhancements#auto|auto类型推导]] 一节中进行介绍。

[](#2-3-类型推导)[]( #2 -3-类型推导 "2.3 类型推导") 2.3 类型推导
---------------------------------------------
...
> 
> ```
> auto auto_arr 2[10] = {arr}; // 错误, 无法推导数组元素类型
> 
> 2.6. auto. cpp:30:19: error: 'auto_arr 2' declared as array of 'auto'
>     auto auto_arr 2[10] = {arr};
> ```
...
### [](#尾返回类型推导)[]( #尾返回类型推导 "尾返回类型推导")尾返回类型推导
...
### [](#decltype-auto)[]( #decltype -auto "decltype (auto)") decltype (auto)

`decltype(auto)` 是 C++14 开始提供的一个略微复杂的用法。

> 要理解它你需要知道 C++ 中参数转发的概念，我们会在[语言运行时强化](https://changkun.de/modern-cpp/zh-cn/03-runtime/index.html)一章中详细介绍，你可以到时再回来看这一小节的内容。
...
[](#2-4-控制流)[]( #2 -4-控制流 "2.4 控制流") 2.4 控制流
-----------------------------------------

### [](#if-constexpr)[]( #if -constexpr "if constexpr") if constexpr
...
### [](#区间-for-迭代)[]( #区间 -for-迭代 "区间 for 迭代")区间 for 迭代
...
### [](#尖括号-quot-gt-quot)[]( #尖括号 -quot-gt-quot "尖括号 ">"")尖括号 ">"
...
### [](#非类型模板参数推导)[]( #非类型模板参数推导 "非类型模板参数推导")非类型模板参数推导
...
[](#2-6-面向对象)[]( #2 -6-面向对象 "2.6 面向对象") 2.6 面向对象
---------------------------------------------
...
### [](#显式虚函数重载)[]( #显式虚函数重载 "显式虚函数重载")显式虚函数重载
...
### [](#显式禁用默认函数)[]( #显式禁用默认函数 "显式禁用默认函数")显式禁用默认函数
...
2.  尝试用[折叠表达式](#%E6%8A%98%E5%8F%A0%E8%A1%A8%E8%BE%BE%E5%BC%8F)实现用于计算均值的函数，传入允许任意参数。
